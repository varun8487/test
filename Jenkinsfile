def call() {
	//Check Parameter
	if (CRED_HONDA_GERRIT_USER.size() < 1) {
		error("no parameter CRED_HONDA_GERRIT_USER.")
		return
	}
	if (CRED_KPIT_GERRIT_USER.size() < 1) {
		error("no parameter CRED_KPIT_GERRIT_USER.")
		return
	}
	...
	//	return
	// }
	//if (CRED_KPIT_SFTP_ACCT.size() < 1) {
	//	error("no parameter CRED_KPIT_SFTP_ACCT.")
	//	return
	// }
...
//	withCredentials([uisernamePassword(credentialsId: "${CRED_DP1188_ACCT}", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
